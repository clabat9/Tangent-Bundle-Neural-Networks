...
% set parameters
Nb = 1200; % number of sampling points
n_real = 10; % number of data (sampling) realizations
sigma_noise_coll = [5e-3,1e-2,5e-2,1e-1,5e-1]; % noise variances to be used
n_noise_real = 30; % number of noisy signal realizations per each sampling realization
add_noise = 1;
savevars = 1;
for rel = 1:n_real
    % Sample 2-sphere
    ...
    data = [U;V;W]; % Clean Data
% To reproduce the paper figure
...
%     cameratoolbar('SetCoordSys', 'none');
    ...
    %% Vector Diffusion Maps Parameters
    vdm = dm;
    vdm.embedmaxdim = 100;
    vdm.debug = 1;
    [rsltDM] = DiffMap(dm);
    ...
    % Projection after Local PCA, noising and saving
