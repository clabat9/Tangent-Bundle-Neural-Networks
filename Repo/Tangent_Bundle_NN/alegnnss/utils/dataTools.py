...
import alegnnss.utils.graphTools as graph
...
class _dataForSemisupervised(_data):
    # Internal supraclass from which data classes inherit when they are used
    # (also) for semisueprvised regression. This renders the .evaluate() method the same in all
    # cases (MSE) so justifies the use of another internal class.
    def __init__(self):
        super().__init__()
        self.test_indices = None
        self.test_indices_torch = None
        self.evaluate_only_test = None
        self.nPoints = None
    def evaluate(self, yHat, y, tol = 1e-9):
        if yHat.shape != y.shape:
            y = y.flatten()
        
        if not self.evaluate_only_test:
            N = self.nPoints#np.prod(y.shape) 
            if 'torch' in repr(self.dataType):
                #   And compute the error
                totalErrors = torch.sum((y - yHat)**2)/N
                errorRate = totalErrors.type(self.dataType)
            else:
                yHat = np.array(yHat)
                y = np.array(y)
                #   And compute the error
                totalErrors = np.sum((yHat - y)**2)/N
                errorRate = totalErrors.astype(self.dataType)
            return errorRate
        else:
            if 'torch' in repr(self.dataType):
                #   And compute the error
                gat1 = torch.gather(yHat,1,self.test_indices_torch) 
                gat2 = torch.gather(y,1,self.test_indices_torch) 
                N = max(gat2.shape)
                totalErrors = torch.sum(torch.abs(gat1 - gat2))/N
                errorRate = totalErrors.type(self.dataType)
            else:
                yHat = np.array(yHat)
                y = np.array(y)
                gat1 = np.gather(yHat,1,self.test_indices) 
                gat2 = np.gather(y,1,self.test_indices) 
                N = max(gat2.shape)
                #   And compute the error
                totalErrors = np.sum(np.abs((yHat - y)))/N
                errorRate = totalErrors.astype(self.dataType)
            return errorRate
