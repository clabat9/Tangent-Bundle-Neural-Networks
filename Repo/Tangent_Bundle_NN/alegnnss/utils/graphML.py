...
import alegnnss.utils.graphTools as graphTools
...
class ReLUwithS(nn.Module):
    """
    ReLUwithS creates an activation layer that takes the input data and Laplacian matrix and 
        return nn.ReLU(x) and the Laplacian matrix
    Initialization:
        ReLUwithS()
    ...
    Forward call:
        ...
        Inputs:
            x (torch.tensor): input data; shape:
                batch_size x dim_features x number_nodes
            S (torch.tensor): input Laplacian matrix of data of shape
                batchSize x edgeFeatures x numberNodes x numberNodes
...
class MaxPoolLocal(nn.Module):
    ...
    def forward(self, x):
        ...
        gatherNeighbor = gatherNeighbor.repeat([batchSize, dimNodeSignals, 1,1]).type(torch.int64)
        # And finally we're in position of getting all the neighbors in line
