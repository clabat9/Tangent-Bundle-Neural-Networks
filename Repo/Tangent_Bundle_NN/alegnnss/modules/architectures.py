...
import alegnnss.utils.graphTools

from alegnnss.utils.dataTools import changeDataType
...
class SelectionGNN(nn.Module):
    ...
    def __init__(self,
        ...
        if order is not None:
            ...
            self.permFunction = eval('alegnnss.utils.graphTools.perm' + order)
        else:
            self.permFunction = alegnnss.utils.graphTools.permIdentity
        ...
        if self.coarsening and self.E == 1:
            self.permFunction = alegnnss.utils.graphTools.permCoarsening # Override
                # permutation function for the one corresponding to coarsening
            GSO = scipy.sparse.csr_matrix(GSO[0])
            GSO, self.order = alegnnss.utils.graphTools.coarsen(GSO, levels=self.L,
    ...
    def splitForward(self, x):
        ...
        if x.shape[2] != self.N[0] and self.coarsening:
            ...
            x = alegnnss.utils.graphTools.permCoarsening(x, self.order)
