...
# This is the code used for implementing the MNN numerical results in the paper:
...
# over the vector field (-y,x,0) tangent to the unitary sphere S2 using MNN
...
thisFilename = 'manifoldnn' # This is the general name of all related files
...
for nPoints in [200,800]:
    ...
    # ARCHITECTURES #
    #################
    # Manifold Neural Networks are implemented via Graph Neural Netwroks; 
    # Sheaf Neural Networks can be implemented via a Selection-GNN with the GSO set
    # as the CLoud Laplacian
    # We exploit a one layered MNN 
    ...
    #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    #\\\ MNN via Selection GNN \\\
    #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    
    #\\\ Basic parameters for both MNN 
    modelSelGNN = {} # Model parameters for the Selection GNN (SelGNN)
    modelSelGNN['name'] = 'MNN'
    ...
    # Layers Parameters
    modelSelGNN['dimNodeSignals'] = [3, 3] # Features per layer
    ...
    modelSelGNN['nSelectedNodes'] = [nPoints] 
    ...
    #\\\ MODEL 1: MNN \\\
    #\\\\\\\\\\\\\\\\\\\\\\\
    modelMNN = deepcopy(modelSelGNN)
    modelMNN['name'] = 'MNN' # Name of the architecture
    # Nonlinearity
    modelMNN['nonlinearity'] = nn.PReLU
    # Save Values:
    writeVarValues(varsFile, modelMNN)
    modelList += [modelMNN['name']]
    ...
    for split in range(nDataSplits):
        for rel in range(nNoiseSplits):
            ...
            # Clearly we have one data matrix, train data are noisy, validation/test is
            ...
            data.samples = {}
            data.samples['train'] = {}
            data.samples['train']['signals'] = torch.from_numpy(train_np)
            data.samples['train']['targets'] = torch.from_numpy(train_np)
            data.samples['valid'] = {}
            data.samples['valid']['signals'] = torch.from_numpy(train_np)
            data.samples['valid']['targets'] = torch.from_numpy(data_np)
            data.samples['test'] = {}
            data.samples['test']['signals'] = torch.from_numpy(train_np)
            data.samples['test']['targets'] = torch.from_numpy(data_np)
            ...
            for thisModel in modelList:
                ...
                thisLossFunction = modelDict.pop('lossFunction')(None, reg_smooth,\
                ...
                # Store it
                modelsMNN[thisName] = modelCreated
            ...
            for thisModel in modelsMNN.keys():
                
                if doPrint:
                    print("Training model %s..." % thisModel)
                
                # Remember that modelsMNN.keys() has the split numbering as well as the
                ...
                # Train the model
                thisTrainVars = modelsMNN[thisModel].train(data,
                                                           nEpochs,
                                                           batchSize,
